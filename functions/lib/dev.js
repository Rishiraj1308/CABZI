...
exports.triggerRide = exports.statusCleanup = exports.emergencyCaseUpdater = exports.dispatchEmergencyCase = exports.dispatchGarageRequest = exports.dispatchRideRequest = void 0;
/**
...
const handleRideDispatch = async (rideData, rideId) => {
    ...
    if (tokens.length > 0) {
        const message = {
            data: Object.assign(Object.assign({ type: 'new_ride_request', rideId }, rideData), { pickupLocation: JSON.stringify(rideData.pickup.location), destinationLocation: JSON.stringify(rideData.destination.location) }),
            tokens: tokens,
        };
...
const handleGarageRequestDispatch = async (requestData, requestId) => {
    ...
    if (tokens.length > 0) {
        const message = {
            data: Object.assign({ type: 'new_garage_request', requestId }, requestData),
            tokens: tokens,
        };
        await messaging.sendEachForMulticast(message);
    }
...
const handleEmergencyDispatch = async (caseData, caseId) => {
    ...
    const availableHospitals = hospitalsSnapshot.docs
        ...
        return Object.assign(Object.assign({}, hospital), { distance });
    })
    ...
    if (hospitalFCMToken) {
        const message = {
            data: Object.assign({ type: 'new_emergency_request', caseId: caseId }, caseData),
        ...
        await db.doc(`emergencyCases/${caseId}`).update({ rejectedBy: firestore_2.FieldValue.arrayUnion(targetHospital.id) });
    }
};
...
exports.triggerRide = handleRideDispatch;
