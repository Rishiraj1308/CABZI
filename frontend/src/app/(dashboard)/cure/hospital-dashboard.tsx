...
import { Ambulance, Siren, UserPlus, Car, Map, BedDouble, Check, XCircle, HospitalIcon } from 'lucide-react'
...
import { collection, doc, onSnapshot, query, updateDoc, where, serverTimestamp, GeoPoint, addDoc, setDoc, arrayUnion } from 'firebase/firestore'
...
const AddStaffDialog = ({ type, session }: { type: 'driver' | 'doctor', session: ClientSession | null }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            toast.success(`New ${type} added successfully!`, { description: 'Their profile is now active.' });
            setIsOpen(false);
            setFormData({});
        } catch (error) {
            console.error(error);
...
export default function HospitalMissionControl({ partnerData, isLoading: isPartnerLoading }: { partnerData: any, isLoading: boolean }) {
    ...
    useEffect(() => {
        if (!isPartnerLoading && partnerData) {
            ...
            setTotalBeds(partnerData.totalBeds || 20);
            setOccupiedBeds(partnerData.bedsOccupied || 5);
        }
    }, [isPartnerLoading, partnerData]);


    // Listeners for fleet and cases
    useEffect(() => {
        if (!db || !partnerData?.id) return;
        
        setIsLoading(true);

        const fleetUnsub = onSnapshot(collection(db, `curePartners/${partnerData.id}/ambulances`), (snapshot) => {
    ...
    }, [db, partnerData?.id, emergencyCases]);
    ...
    const handleBedUpdate = async (type: 'total' | 'occupied', value: number) => {
        if (!db || !partnerData?.id || value < 0) return;
        ...
        if (type === 'total') {
            ...
            fieldToUpdate = { totalBeds: value };
             ...
             if (value > totalBeds) {
                toast.error('Invalid Input', { description: 'Occupied beds cannot exceed total beds.' });
                return;
            }
        ...
        try {
            await updateDoc(doc(db, 'curePartners', partnerData.id), fieldToUpdate);
    ...
    const handleCaseAction = async (caseId: string, action: 'accept' | 'reject') => {
        ...
        try {
            if (action === 'accept') {
                await updateDoc(caseRef, { status: 'accepted', 'assignedPartner.name': partnerData.name });
                toast.success(`Case ${caseId} Accepted`);
            } else {
            ...
            toast.error(`Action failed for Case ${caseId}`);
        }
    }
    
    const availableBeds = totalBeds - occupiedBeds;
    const pendingCases = useMemo(() => emergencyCases.filter(c => c.status === 'pending'), [emergencyCases]);
    const activeCases = useMemo(() => emergencyCases.filter(c => c.status !== 'pending' && c.status !== 'completed' && c.status !== 'cancelled_by_rider'), [emergencyCases]);
        ...
        <div className="space-y-6">
             <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <p className="text-muted-foreground">{partnerData?.name || 'Hospital Dashboard'}</p>
                </div>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <Siren className={cn("w-6 h-6", pendingCases.length > 0 && "text-destructive animate-pulse-intense")}/>
                                Action Feed
                            </CardTitle>
                                    ...
                                    >
                                        <Alert variant="destructive" className="mb-4">
                                            ...
                                            <AlertTitle className="flex justify-between items-center">
                                                ...
                                                <Badge className="capitalize">{c.severity}</Badge>
                                            </AlertTitle>
                                            <AlertDescription className="mt-1">
                                                From {c.riderName} ({c.phone}). Tap to view location on map.
                                            </AlertDescription>
                                            <div className="mt-4 flex gap-2">
                                                <Button size="sm" className="flex-1" onClick={() => handleCaseAction(c.id, 'accept')}><Check className="w-4 h-4 mr-2"/> Accept</Button>
                                                <Button size="sm" variant="outline" className="flex-1" onClick={() => handleCaseAction(c.id, 'reject')}><XCircle className="w-4 h-4 mr-2"/> Reject</Button>
                                            </div>
                                        </Alert>
                                    </motion.div>
                                ))
                     ...
                     <Card>
                        ...
                        <CardContent className="h-[400px] w-full p-0">
                             <LiveMap activePartners={fleet.filter(v => v.currentLocation).map(v => ({ id: v.id, name: v.name, type: 'ambulance', location: { latitude: v.currentLocation!.latitude, longitude: v.currentLocation!.longitude },}))} />
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="occupied-beds" type="number" value={occupiedBeds} onChange={e => setOccupiedBeds(parseInt(e.target.value))} max={totalBeds} min={0} />
