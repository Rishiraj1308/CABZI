...
export default function OnboardingPage() {
    ...
    const validateStep1 = () => {
        if (!formData.name || !formData.phone || !formData.gender || !formData.dob) {
            toast.error("Incomplete Details", { description: "Please fill out your name, phone, gender and date of birth." });
            return false;
        }
    ...
    const validateStep2 = () => {
         if (!formData.panCard || !formData.aadhaarNumber) {
            toast.error("Incomplete Details", { description: "Please enter your PAN and Aadhaar number." });
            return false;
        }
        const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
        if (!panRegex.test(formData.panCard.toUpperCase())) {
            toast.error('Invalid PAN Format', { description: 'Please enter a valid PAN card number (e.g., ABCDE1234F).' });
            return false;
        }
        const aadhaarRegex = /^\d{12}$/;
        if (!aadhaarRegex.test(formData.aadhaarNumber)) {
            toast.error('Invalid Aadhaar Format', { description: 'Please enter a valid 12-digit Aadhaar number.' });
            return false;
        }
        return true;
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.vehicleType || !formData.vehicleBrand || !formData.vehicleName || !formData.vehicleNumber || !formData.drivingLicence) {
             toast.error("Incomplete Form", { description: "Please fill all vehicle details on the final step." });
             setIsLoading(false);
             return;
        }
        ...
        try {
            ...
            const partnerDocRef = doc(db, "pathPartners", systematicId);
            ...
            toast.success("Onboarding Successful!", { description: `Welcome, ${name}! Your partner ID is ${systematicId}. Redirecting to login...` });
