...
function UserLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  if(!user) {
    return <div className="flex h-screen w-full items-center justify-center"><Skeleton className="h-screen w-screen" /></div>;
  }
            ...
            <>
                ...
                <footer className="fixed bottom-0 left-0 right-0 z-40 p-4">
                  <div className="relative mx-auto max-w-sm">
                    <nav className="flex items-center justify-around h-16 rounded-full bg-background/80 backdrop-blur-lg border border-border shadow-lg">
                      {navItems.map((item) => (
                          <Link 
                            key={item.href} 
                            href={item.href} 
