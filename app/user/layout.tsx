...
function LanguageToggle() {
    ...
    return (
        <Button variant="ghost" size="icon" onClick={() => setLanguage(language === 'en' ? 'hi' : 'en')} className="inline-flex items-center justify-center h-10 w-10 rounded-full border border-white/10 bg-white/5 backdrop-blur hover:bg-white/10" aria-label="Switch language" title="Switch language">
...
function LocationDisplay() {
  ...
  return (
    <div className="hidden md:flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm backdrop-blur">
      <MapPin className="h-4 w-4 text-white/60" />
      <span className="font-medium text-white/80 truncate">{locationAddress}</span>
    </div>
  );
}
...
function ThemeToggle() {
    ...
    return (
      <Button variant="ghost" size="icon" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="inline-flex items-center justify-center h-10 w-10 rounded-full border border-white/10 bg-white/5 backdrop-blur hover:bg-white/10" aria-label="Toggle theme" title="Toggle theme">
...
export default function UserLayout({
  ...
  const OuterContainer = ({ children }: { children: React.ReactNode }) => (
     <div className={cn("h-full min-h-screen antialiased text-white aurora-background")} style={{fontFamily: "Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, 'Helvetica Neue', 'Noto Sans', 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol', 'sans-serif'"}}>
      {children}
    </div>
  )
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <nav className="flex items-center justify-between py-4">
                        <SheetTrigger asChild>
                            <Button id="openDrawer" className="inline-flex md:hidden items-center justify-center h-10 w-10 rounded-full border border-white/10 bg-white/5 backdrop-blur hover:bg-white/10 focus:outline-none focus-visible:outline focus-visible:outline-2 focus-visible:outline-white/30" aria-label="Open menu" aria-controls="drawer" aria-expanded={isMobileMenuOpen} title="Open menu">
                        ...
                        <div className="ml-auto flex items-center gap-2">
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                 <Button variant="ghost" size="icon" className="rounded-full w-10 h-10 border border-white/10 bg-white/5 backdrop-blur hover:bg-white/10">
