...
import type { AmbulanceCase, GarageRequest, ClientSession } from '@/models/types';
...
export default function EmergencyButtons({ serviceType, pickupCoords, setIsRequestingSos, setActiveAmbulanceCase, setActiveGarageRequest, onBack, session }: EmergencyButtonsProps) {
  ...
  const handleFindHospitals = async () => {
    ...
    if (!db || !pickupCoords) {
        toast({ variant: 'destructive', title: 'Database or Location Error' });
        return;
    }
  ...
  const handleConfirmAmbulanceRequest = async () => {
    if (!selectedHospital || !db || !session || !severity || !pickupCoords) {
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent>
            <DialogHeader>
                <Button variant="ghost" size="icon" className="absolute top-3 left-3" onClick={onBack}><ArrowLeft className="w-5 h-5"/></Button>
