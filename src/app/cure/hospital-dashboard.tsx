...
import { Ambulance, Check, X, PlusCircle, Car, User, Siren, Map, Waves, Settings, ServerCrash, BedDouble, BarChart, Clock, Users as UsersIcon, KeyRound, Navigation, UserPlus, Phone, Share2, MoreHorizontal, Trash2, ListChecks, FileText, Minus, Plus, Hospital, Calendar, PersonStanding } from 'lucide-react'
...
export default function HospitalMissionControl({ 
    ...
    const simulateNewCase = async () => {
        ...
        const testCaseData = {
            ...
            phone: "+919999988888",
            severity: 'Critical',
            location: partnerData.location ? new GeoPoint(partnerData.location.latitude + 0.01, partnerData.location.longitude + 0.01) : new GeoPoint(28.6139, 77.2090),
            status: 'pending',
            otp: generatedOtp,
    ...
    useEffect(() => {
        ...
        const fetchAnalytics = async () => {
            ...
            const fleetSnapshot = await getDocs(collection(db, `ambulances/${partnerData.id}/fleet`));
     ...
     useEffect(() => {
        if (ongoingCase?.assignedAmbulanceId && partnerData?.id && db) {
            const ambulanceRef = doc(db, `ambulances/${partnerData.id}/fleet`, ongoingCase.assignedAmbulanceId);
    ...
    }, [ongoingCase, partnerData?.id, db]);

    const handleOnlineStatusChange = async (checked: boolean) => {
        if (!partnerData?.id || !db) return;
        setIsOnline(checked);
        const hospitalRef = doc(db, "ambulances", partnerData.id);
        try {
    ...
    const handleAddDriver = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const driversRef = collection(db, `ambulances/${partnerData.id}/drivers`);
        ...
        try {
            ...
            batch.set(globalDriverDocRef, {
                ...
                status: 'Active', hospitalId: partnerData.id, hospitalName: partnerData.name,
    ...
    const handleAcceptRequest = async (request: EmergencyRequest, ambulanceId: string) => {
        ...
        const ambulanceRef = doc(db, `ambulances/${partnerData.id}/fleet`, ambulanceId);
        ...
        try {
            ...
            await updateDoc(requestRef, {
                ...
                hospitalLocation: partnerData.location,
            });
    ...
    const handleDeleteAmbulance = async (ambulance: AmbulanceVehicle) => {
        ...
        const ambulanceRef = doc(db, `ambulances/${partnerData.id}/fleet`, ambulance.id);
        try {
                                        ...
                                        {selectedDriver && (<div className="space-y-4 py-4">
                                            <Card className="bg-gradient-to-br from-primary/90 to-primary text-primary-foreground p-4 rounded-lg shadow-lg">
                                                ...
                                                <div className="mt-4 pt-4 border-t border-primary-foreground/30 space-y-2 text-sm">
                                                    <div className="flex justify-between"><span>Hospital:</span><span className="font-semibold">{partnerData?.name}</span></div>
