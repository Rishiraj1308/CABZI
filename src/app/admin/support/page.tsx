...
import { collection, query, getDocs, orderBy, Timestamp, doc, updateDoc, arrayUnion, onSnapshot } from 'firebase/firestore'
...
export default function AdminSupportPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
        const queriesData: SupportQuery[] = [];
        querySnapshot.forEach((doc) => {
            queriesData.push({ id: doc.id, ...doc.data() } as SupportQuery);
        });
        setQueries(queriesData);
        setIsLoading(false);
    }, (error) => {
        console.error("Error fetching support queries: ", error);
        toast({ variant: 'destructive', title: 'Error', description: 'Could not fetch support queries.' });
        setIsLoading(false);
    });
