...
import {
    ...
    IndianRupee, MapPin, Video, Building, X, Baby, Thermometer, Bone, BrainCircuit, Heart, Ear, Layers, SlidersHorizontal, AlertTriangle, HeartPulse
} from 'lucide-react';
...
export default function BookAppointmentPage() {
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 md:p-6 space-y-8">
            <div className="text-center max-w-2xl mx-auto">
                ...
                <div className="relative mt-6 max-w-xl mx-auto">
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => { if(e.key === 'Enter') { setSelectedSymptom(null); setStep(2); }}}
                    />
                </div>
            </div>
            <div className="space-y-4">
                    ...
                    <CarouselContent className="-ml-2">
                        {symptomCategories.map((symptom, i) => (
                           <CarouselItem key={i} className="pl-2 basis-1/3 md:basis-1/4 lg:basis-1/6">
                                ...
                                <Card className="p-4 flex flex-col items-center justify-center gap-2 cursor-pointer hover:ring-2 hover:ring-primary transition-all text-center h-full" onClick={() => { setSelectedSymptom(symptom.name); setSearchQuery(''); setStep(2); }}>
             ...
             <div className="space-y-4">
                <Label className="font-semibold text-lg">Doctors Near You</Label>
                ...
                <Button variant="outline" className="w-full" onClick={() => setStep(2)}>View All Doctors</Button>
            </div>
          </motion.div>
        );
      case 2:
                    ...
                    <div className="relative flex-1">
                     ...
                     <Sheet open={isFilterSheetOpen} onOpenChange={setIsFilterSheetOpen}>
                        ...
                        <SheetContent>
                            ...
                            <SheetFooter><Button onClick={() => setIsFilterSheetOpen(false)}>Apply Filters</Button></SheetFooter>
                        </SheetContent>
                    </Sheet>
                </div>
            </CardHeader>
                  ...
                  : filteredAndSortedDoctors.length > 0 ? (
                    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-3">
                      {filteredAndSortedDoctors.map(doctor => (
                          <motion.div key={doctor.id} variants={itemVariants} whileHover={{ scale: 1.02 }}>
                            <Card className="p-4 flex gap-4 items-start cursor-pointer hover:bg-muted" onClick={() => { setSelectedDoctor(doctor); setStep(3); }}>
                                <Avatar className="w-20 h-20"><AvatarImage src={doctor.photoUrl || `https://i.pravatar.cc/150?u=${doctor.id}`} /><AvatarFallback>{doctor.name.substring(0,2)}</AvatarFallback></Avatar>
                                <div className="flex-1">
                                    <p className="font-bold text-lg">Dr. {doctor.name}</p>
                                    <p className="font-semibold text-primary">{doctor.specialization}</p>
