...
export default function DriverDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribePartner = onSnapshot(q, (querySnapshot) => {
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [isOnline, partnerData, db, acceptedRide, activeRideRequest]);
  ...
  const handleAcceptRide = async () => {
    if (!activeRideRequest || !db || !partnerData || !partnerData.id) {
        toast({
            ...
            description: 'Your partner details could not be loaded. Please try again.',
        });
        return;
    }
    ...
    const livePartnerData = partnerSnap.data();
    ...
    if (activeGarageRequest) {
     ...
     return (
       <div className="flex-1 flex flex-col gap-4">
            {isOnline ? (
                <Alert className="text-center bg-transparent border-none p-0">
                    <SearchingIndicator partnerType="path" className="w-16 h-16"/>
                    <AlertTitle className="mt-2 font-bold">Looking for rides...</AlertTitle>
                    <AlertDescription>You are online and ready to accept new trips.</AlertDescription>
                </Alert>
            ) : (
