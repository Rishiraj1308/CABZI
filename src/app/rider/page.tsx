...
import { collection, addDoc, serverTimestamp, doc, GeoPoint, query, where, getDocs, updateDoc, getDoc, deleteDoc } from 'firebase/firestore'
...
export default function RiderPage() {
  ...
  useEffect(() => {
    ...
    const checkActiveGarageRequest = async () => {
      const requestId = localStorage.getItem('activeGarageRequestId');
      if (requestId && db) {
          const requestRef = doc(db, 'garageRequests', requestId);
 ...
 const getCoordinates = async (address: string): Promise<{ lat: number; lon: number } | null> => {
      ...
      try {
            ...
            if (result && result.routes && result.routes.length > 0) {
                ...
                return {
                    ...
                    duration: Math.round(route.summary.travelTimeInSeconds / 60 * 1.2),
                    geometry: route.legs.reduce((acc: any, leg: any) => {
                      if (leg.points) {
                        const coords = leg.points.map((p: any) => [p.longitude, p.latitude]);
                        return { type: "LineString", coordinates: [...(acc.coordinates || []), ...coords]};
                      }
                      return acc;
  ...
  const handleGetRideInfo = async () => {
    ...
    // This part is changed. Instead of querying Firestore, we will just simulate a number.
    // In a real app, this would be an API call to our backend which queries PostgreSQL.
    setNearbyDrivers(Math.floor(Math.random() * 10) + 5); 
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(async () => {
             ...
             if (docSnap.exists()) {
                ...
                if(data.status !== activeAmbulanceCase.status){
                     setActiveAmbulanceCase(prev => prev ? {...prev, status: data.status} : null);
                }
    ...
    }, [activeAmbulanceCase, db, pickup.coords]);
  ...
  const renderActiveServices = () => {
    if (activeRide) {
        return <RideStatus ride={activeRide} rating={rating} setRating={setRating} onCancel={handleCancelRideByRider} onPayment={handlePayment} onDone={resetState} />;
    }
    if (activeAmbulanceCase) {
        return <RideStatus ride={activeAmbulanceCase as any} onCancel={resetState} onDone={resetState} />;
    }
    if (activeGarageRequest) {
        return <RideStatus ride={activeGarageRequest as any} isGarageRequest onCancel={resetState} onDone={resetState} />;
    }
    return null;
  }
    ...
    <div className="h-screen w-screen flex flex-col">
        ...
        <div className="flex-1 relative">
            <LiveMap
                ...
                riderLocation={currentUserLocation ?? undefined}
        ...
        {!activeService && renderRideBooking()}
