...
export default function DriverDashboardPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(ridesQuery, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                const rideTypeForMatching = (ride.rideType || "").split(" ")[0].trim();

                if (!ride.rejectedBy?.includes(partnerData.id!) && rideTypeForMatching === vehicleTypeBase) {
    ...
    // Check for active ride on page load and listen for its updates
    useEffect(() => {
        if (!db) return;
        ...
        const unsub = onSnapshot(rideRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (['completed', 'cancelled_by_driver', 'cancelled_by_rider'].includes(rideData.status)) {
                    // Show the final state, then clear it after a delay
                     setActiveRide(rideData); 
                } else {
                    setActiveRide(rideData);
                }
            } else {
                // The ride document was deleted or doesn't exist anymore
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(ridesQuery, (snapshot) => {
            setRecentRides(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as RideData)));
    ...
    <div className="space-y-6">
                    ...
                    <TabsContent value="history" className="mt-4 flex-1 space-y-2 max-h-48 overflow-y-auto">
                          {isHistoryLoading ? (
                              ...
                              recentRides.map(ride => (
                                <Card key={ride.id} className="cursor-pointer hover:bg-muted" onClick={() => setSelectedRide(ride)}>
        ...
        <RidePopup jobRequest={jobRequest} onAccept={handleAcceptJob} onDecline={handleDeclineJob} />

        <Dialog open={selectedRide !== null} onOpenChange={(isOpen) => !isOpen && setSelectedRide(null)}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Ride Details</DialogTitle>
                    {selectedRide && <DialogDescription>{format(selectedRide.createdAt.toDate(), 'PPP, p')}</DialogDescription>}
                </DialogHeader>
                {selectedRide && (
                     <div className="space-y-4 py-4 text-sm">
                        <div className="flex items-start gap-3"><MapPin className="w-4 h-4 mt-1 text-green-500"/><p><span className="font-semibold text-muted-foreground">FROM: </span>{selectedRide.pickup?.address}</p></div>
                        <div className="flex items-start gap-3"><Route className="w-4 h-4 mt-1 text-red-500"/><p><span className="font-semibold text-muted-foreground">TO: </span>{selectedRide.destination?.address}</p></div>
                        <Separator/>
                        <div className="flex justify-between items-center"><span className="text-muted-foreground">Rider</span><span className="font-semibold">{selectedRide.riderName}</span></div>
                        <div className="flex justify-between items-center"><span className="text-muted-foreground">Status</span><span className="font-semibold">{selectedRide.status}</span></div>
                        <div className="flex justify-between items-center text-lg"><span className="text-muted-foreground">Fare</span><span className="font-bold text-primary">₹{selectedRide.fare}</span></div>
                    </div>
                )}
            </DialogContent>
        </Dialog>
        ...
        <Dialog open={isPinDialogOpen} onOpenChange={setIsPinDialogOpen}>
            <DialogContent className="max-w-xs">
                ...
                <DialogFooter><Button type="button" className="w-full" onClick={handlePinSubmit}>Submit</Button></DialogFooter>
            </DialogContent>
        </Dialog>
    </div>
    );
