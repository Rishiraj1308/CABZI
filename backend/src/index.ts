...
const handleRideDispatch = async (rideData: any, rideId: string) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const nearbyPartners = partnersSnapshot.docs
            ...
            .filter(p => {
                if (!p.currentLocation || !p.vehicleType || !p.vehicleType.startsWith(rideTypeBase) || rejectedBy.includes(p.id)) return false;
        ...
        if (tokens.length > 0) {
             const payloadData = {
                ...
                vehicleNumber: targets[0].vehicleNumber ?? "N/A", // Correctly include vehicleNumber
             };
